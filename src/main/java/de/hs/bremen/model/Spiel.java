...
public class Spiel {
	...
	/**
	 * Spielfelder f√ºr die einzelnen Spieler werden erstellt
	 * @param groesse: Gr√∂√üe der Spielfelder
	 */
	...
	public void schiffePlatzieren(){
		...
		do{
			System.out.println("Bitte geben Sie an mit vielen Schiffen Sie pro Person spielen mÔøΩchten.");
		...
		for (int i = 0 ; i< spieler.length; i++){		
			...
			do {				
				System.out.println(spieler[i].getName() +",Bitte w‰hlen Sie ein Schiff, welches Sie auf dem Spielfeld platzieren wollen.");
				System.out.println("[1 f¸r Zerstˆrer, 2 f¸r Fregatte, 3 f¸r Korvette, 4 f¸r UBoot]");
	...
	/**
	 * Gibt wieder, ob das im Parameter √ºbergebene Schiff, noch auf das Spielfeld des Spielers passt.
	 * @param spieler: Spieler welches Spielfeld √ºberpr√ºft werden soll
	 * @param schiff: Schiff, dessen L√§nge √ºberpr√ºft werden soll
	...
	public boolean anzahlSchiffePassend(Spieler spieler, Schiff schiff){
		...
		if(!passend){
			System.out.println("Dieses Schiff passt leider nicht mehr auf ihr Spielfeld. Bitte w‰hlen Sie ein anderes");
		}
		return passend;
	}
	...
	public void init(){
		...
		System.out.println("Bitte geben Sie zunÔøΩchst die Anzahl der Spieler an");
		createSpieler(IO.readInt());
		System.out.println("Einigen Sie sich nun bitte auf eine Grˆﬂe ihres quadratischen Spielfelder (Mindestens 20x20 Felder groﬂ)");
		System.out.println("Wie groﬂ soll ihr Spielfeld sein?");
		groesse = IO.readInt();
